...
onError: () => {
  toast.error("The transfer could not be returned. Refresh the page.");
},
});
...
onMutate: async (id) => {
  ...
  const previous = queryClient.getQueryData<InfiniteData<LikedTranslationsResponse>>(QUERY_KEY);
  ...
  if (previous) {
	...
	queryClient.setQueryData<InfiniteData<LikedTranslationsResponse>>(QUERY_KEY, {
	  ...previous,
	  pages,
	});
  }
...
onSuccess: (_id, _vars, ctx) => {
  ...
  const restore = () => {
	...
	queryClient.setQueryData<InfiniteData<LikedTranslationsResponse>>(QUERY_KEY, (current) => {
	  if (!current) return current;
	  const pages = current.pages.map((page, pi) => {
		if (pi === pageIndex) {
		  const arr = [...page.translations];
		  const pos = Math.min(Math.max(itemIndex, 0), arr.length);
      ...
      {isLoading && flatTranslations.length === 0 ? (
        ...
        <div className="flex-1 flex flex-col items-center justify-center text-center px-4">
          ...
          <p className="text-xs text-muted-foreground max-w-sm sm:text-sm">
            Start translating and like your favorite results to see them here
          </p>
        </div>
      ) : (
        <>
