...
interface LikedTransCardProps {
  translation: Translation;
  onCopy: (text: string) => void;
  onDelete: (translationId: string) => void;
}

const LikedTransCard = ({ translation, onCopy, onDelete }: LikedTransCardProps) => {
  ...
  const srcLabel =
    languages[translation.sourceLang as keyof typeof languages]?.label ?? translation.sourceLang;
  const tgtLabel =
    languages[translation.targetLang as keyof typeof languages]?.label ?? translation.targetLang;
    ...
    <div className="w-full bg-card rounded-lg p-3 border border-border shadow-md sm:p-4">
      ...
      <div className="flex space-y-2 mb-3 items-start justify-between space-y-0 md:mb-4">
        <div className="flex items-center space-x-1 text-xs text-muted-foreground sm:space-x-2 sm:text-sm">
          ...
          <span className="uppercase truncate">{srcLabel}</span>
          <span>→</span>
          <span className="uppercase truncate">{tgtLabel}</span>
        </div>
      ...
      <div className="space-y-3 sm:space-y-4 md:grid md:grid-cols-2 md:gap-4 md:space-y-0">
        <div>
          <p className="text-xs text-muted-foreground mb-1 font-medium sm:text-sm">Source</p>
          <div className="relative bg-muted/50 rounded-md p-2 border border-border sm:p-3 sm:pr-8">
            <p className="text-xs leading-relaxed sm:text-sm">{translation.sourceText}</p>
        ...
        <div>
          <p className="text-xs text-muted-foreground mb-1 font-medium sm:text-sm">Translation</p>
          <div className="relative bg-muted/50 rounded-md p-2 border border-border sm:p-3 sm:pr-8">
            <p className="text-xs leading-relaxed sm:text-sm">{translation.resultText}</p>
            <div className="absolute top-2 right-2">
              <button
