...
import { Settings, Heart, User, Bell, Save, Loader2, CheckCircle } from "lucide-react";
import LikedTranslations from "./LikedTranslations";
import { languages, toneStyle, LanguageShort, Tone } from "@/shared/config/translation";
import {
  Select,
  SelectContent,
  SelectItem,
  ...
  const renderTabContent = () => {
    switch (activeTab) {
      case "general":
        return (
          <div className="flex flex-col space-y-4 sm:space-y-6">
            <h2 className="text-lg font-bold sm:text-xl md:text-2xl">General Settings</h2>
            <div className="space-y-4 sm:space-y-6">
              ...
              <div className="space-y-2">
                ...
                <Select value={targetLanguage || "en"} onValueChange={(value) => setTargetLanguage(value as LanguageShort)}>
                  <SelectTrigger className="w-full h-10 text-sm">
              ...
              <div className="space-y-2">
                ...
                <Select value={translationStyle || "natural"} onValueChange={(value) => setTranslationStyle(value as Tone)}>
                  <SelectTrigger className="w-full h-10 text-sm">
              ...
              <div className="flex flex-col space-y-3 pt-4 border-t border-border sm:flex-row sm:items-center sm:justify-between sm:space-y-0 sm:pt-6">
                ...
                >
                  ...
                  <span className="hidden xs:inline">{isSaving ? "Saving..." : "Save Settings"}</span>
      ...
      case "account":
        return (
          <div className="flex flex-col space-y-4 sm:space-y-6">
            <h2 className="text-lg font-bold sm:text-xl md:text-2xl">Account Settings</h2>
            <div className="space-y-4 sm:space-y-6">
              <div className="bg-card rounded-lg p-3 border border-border shadow-sm sm:p-4">
                <h3 className="font-semibold mb-2 text-foreground text-sm sm:text-base sm:mb-3">Profile Information</h3>
                <div className="space-y-2 text-xs sm:text-sm">
                  <div className="flex flex-col space-y-1 sm:flex-row sm:space-y-0 sm:space-x-2">
                    <span className="text-muted-foreground font-medium min-w-0 sm:min-w-[60px]">Name:</span> 
                    <span className="text-foreground truncate">{user.name}</span>
                  </div>
                  <div className="flex flex-col space-y-1 sm:flex-row sm:space-y-0 sm:space-x-2">
                    <span className="text-muted-foreground font-medium min-w-0 sm:min-w-[60px]">Email:</span> 
                    <span className="text-foreground truncate">{user.email}</span>
                  </div>
                  <div className="flex flex-col space-y-1 sm:flex-row sm:space-y-0 sm:space-x-2">
                    <span className="text-muted-foreground font-medium min-w-0 sm:min-w-[60px]">Role:</span> 
                    <span className="text-foreground capitalize">{user.role}</span>
                  </div>
                </div>
              </div>
              <div className="bg-card rounded-lg p-3 border border-border shadow-sm sm:p-4">
                <h3 className="font-semibold mb-2 text-foreground text-sm sm:text-base">Connected Accounts</h3>
                <p className="text-xs text-muted-foreground sm:text-sm">Manage your connected social accounts</p>
              </div>
            </div>
          </div>
        );
      case "notifications":
        return (
          <div className="flex flex-col space-y-4 sm:space-y-6">
            <h2 className="text-lg font-bold sm:text-xl md:text-2xl">Notification Settings</h2>
            <div className="space-y-4 sm:space-y-6">
              <div className="flex flex-col space-y-3 p-3 bg-card rounded-lg border border-border sm:flex-row sm:items-center sm:justify-between sm:space-y-0 sm:p-4">
                <div className="flex-1 min-w-0">
                  <h3 className="font-medium text-foreground text-sm sm:text-base">Email Notifications</h3>
                  <p className="text-xs text-muted-foreground sm:text-sm">Receive updates via email</p>
                </div>
              ...
              <div className="flex flex-col space-y-3 p-3 bg-card rounded-lg border border-border sm:flex-row sm:items-center sm:justify-between sm:space-y-0 sm:p-4">
                <div className="flex-1 min-w-0">
                  <h3 className="font-medium text-foreground text-sm sm:text-base">Translation Reminders</h3>
                  <p className="text-xs text-muted-foreground sm:text-sm">Daily translation practice reminders</p>
                </div>
                <label className="relative inline-flex items-center cursor-pointer self-end sm:self-auto">
                  <input 
                    type="checkbox" 
                    ...
                    onChange={(e) => setTranslationReminders(e.target.checked)}
                  />
                  <div className="w-11 h-6 bg-muted rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-primary"></div>
                </label>
              </div>
            ...
            return (
              ...
              >
                ...
                <span className="text-xs font-medium xs:hidden">
                  {tab.id === "general" ? "Settings" : 
                   tab.id === "liked" ? "Liked" :
                   tab.id === "account" ? "Account" :
                   tab.id === "notifications" ? "Alerts" : tab.label}
                </span>
              </button>
            );
          })}
