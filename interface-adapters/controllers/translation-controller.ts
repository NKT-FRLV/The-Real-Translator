	...
	async handleStreamTranslation(req: NextRequest): Promise<Response> {
		try {
			...
			req.signal?.addEventListener("abort", () => {
				console.log(
					"🔌 Client disconnected - aborting translation request"
				);
					...
					try {
						...
						for await (const chunk of translationStream) {
							...
							if (abortController.signal.aborted) {
								console.log(
									"🛑 Controller: Breaking stream due to abort signal"
								);
								break;
							}
							...
							try {
								...
								if (chunk.type !== "delta") {
									console.log(
										"📤 Controller: Sent SSE chunk:",
										chunk.type
									);
								}
							} catch (error: unknown) {
								console.log(
									"⚠️ Controller: Error sending chunk:",
									error
								);
								...
								if (isConnectionError || isAbortError) {
									console.log(
										"✅ Controller: Connection error is normal (client disconnected)"
									);
						...
						console.log(
							"🏁 Controller: Translation stream completed"
						);
					} catch (error) {
